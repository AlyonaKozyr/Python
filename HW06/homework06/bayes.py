import math
from scraputils import sep_row
...
class NaiveBayesClassifier:
    ...
    def fit(self, x, y):  # x список строк, у список меток
        ...
        words_on_labels = [sum(self.attrs[i + 1]) for i in range(
            classes)]


        for row in range(len(self.attrs[
                                 0])):
            for col in range(classes + 1, classes * 2 + 1):
